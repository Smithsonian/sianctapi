...
jQuery(document).ready(function(){
  ...
  jQuery('a.insert-file').click(function(ev) {
    ...
    try {
      ...
      */
      var alt = jQuery('input#alt').val();
      if (!alt) {
        alt = jQuery(this).data('name');
      }
      ...
      if(!local_url) {
        if (uri.indexOf('ids.si.edu') < 0) {
          uri = 'https://ids.si.edu/ids/deliveryService?max=' + width + '&id=' + uri;
        } else {
      ...
      if (returnPath == 'ckeditor') {
        ...
        window.opener.edan_edanSetFile(window.opener.__edanSelectedFile, window);
      }
      //window.close();
    } catch (err) {
      console.log(err);
